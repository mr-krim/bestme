<script lang="ts">
  ...
  async function loadSavedProviders() {
    try {
      savedProviders = await window.electronAPI.invoke('list_saved_api_keys');
    } catch (error) {
  ...
  async function loadAvailableModels() {
    try {
      availableModels = await window.electronAPI.invoke('list_available_models');
    } catch (error) {
  ...
  async function loadDownloadedModels() {
    try {
      downloadedModels = await window.electronAPI.invoke('list_downloaded_models');
    } catch (error) {
  ...
  async function saveApiKey() {
    ...
    isLoading = true;
    try {
      await window.electronAPI.invoke('save_api_key', {
  ...
  async function deleteApiKey(provider: string) {
    ...
    try {
      await window.electronAPI.invoke('delete_api_key', { provider });
      await loadSavedProviders();
  ...
  async function initializeAI() {
    ...
    try {
      if (selectedProvider === 'local') {
        ...
        await window.electronAPI.invoke('init_local_ai', {
          request: {
        ...
        await window.electronAPI.invoke('init_cloud_ai', {
          request: {
            provider: selectedProvider,
  ...
  async function downloadModel(modelName: string) {
    isLoading = true;
    try {
      await window.electronAPI.invoke('download_model', { modelName });
  ...
  async function testEnhancement() {
    try {
      const result = await window.electronAPI.invoke('enhance_text', {
        request: {
