<script lang="ts">
  ...
  onMount(async () => {
    try {
      ...
      gpuEnabled = await window.electronAPI.invoke('is_gpu_enabled');
      
      if (gpuEnabled) {
        // Get GPU info
        gpuInfo = await window.electronAPI.invoke('get_gpu_info');
        
        // Get available backends
        availableBackends = await window.electronAPI.invoke('get_available_gpu_backends');
      }
    } catch (error) {
