<script lang="ts">
  ...
  async function loadCustomModels() {
    ...
    try {
      customModels = await window.electronAPI.invoke('list_custom_models');
      sortModels();
    } catch (e) {
  ...
  async function saveEdits() {
    ...
    try {
      ...
      await window.electronAPI.invoke('update_custom_model', {
  ...
  async function deleteModel(model: CustomModel) {
    ...
    try {
      await window.electronAPI.invoke('delete_custom_model', { modelId: model.id });
  ...
  async function exportModel(model: CustomModel) {
    try {
      // File dialog functionality would use Electron's dialog API
      // For now, log the export request
      console.log('Export model requested:', model.name);
      toast.push('Export functionality not yet implemented in Electron version');
    } catch (e) {
  ...
  async function loadModelForUse(model: CustomModel) {
    try {
      await window.electronAPI.invoke('load_ai_model', { modelId: model.id });
