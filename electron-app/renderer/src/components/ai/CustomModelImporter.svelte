<script lang="ts">
  ...
  // Dialog functionality implemented using Electron APIs
  ...
  async function validateModel() {
    ...
    try {
      validationResult = await window.electronAPI.invoke('validate_onnx_model', {
        path: selectedFile
      });
  ...
  async function importModel() {
    ...
    try {
      importResult = await window.electronAPI.invoke('import_custom_model', {
        path: selectedFile,
  ...
  async function deleteModel(modelId: string) {
    ...
    try {
      await window.electronAPI.invoke('delete_custom_model', { modelId });
      await loadCustomModels();
    } catch (e) {
  ...
  async function exportModel(modelId: string) {
    try {
      ...
      if (savePath && typeof savePath === 'string') {
        await window.electronAPI.invoke('export_custom_model', {
          modelId,
          exportPath: savePath
        });
      }
