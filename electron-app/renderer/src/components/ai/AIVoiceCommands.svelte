<script lang="ts">
  ...
  onMount(async () => {
    ...
    commandListener = await window.electronAPI.listen('voice-command:detected', (event: any) => {
      if (aiEnabled) {
  ...
  async function loadSettings() {
    try {
      const settings = await window.electronAPI.invoke('get_ai_voice_settings');
      if (settings) {
  ...
  async function saveSettings() {
    try {
      await window.electronAPI.invoke('save_ai_voice_settings', {
        settings: {
          enabled: aiEnabled,
  ...
  async function processAICommand(command: any) {
    ...
    try {
      ...
      const result = await window.electronAPI.invoke('process_ai_voice_command', {
        text: command.text,
