<script lang="ts">
  ...
  onMount(async () => {
    ...
    const unlistenProgress = await window.electronAPI.listen('model-download-progress', (event: any) => {
    ...
    const unlistenComplete = await window.electronAPI.listen('model-download-complete', async (event: any) => {
  ...
  async function downloadModel(modelId: string) {
    try {
      await window.electronAPI.invoke('download_model', { modelId });
  ...
  async function toggleModelLoad(modelId: string) {
    ...
    try {
      if (isLoaded) {
        await window.electronAPI.invoke('unload_ai_model', { modelId });
      } else {
        await window.electronAPI.invoke('load_ai_model', { modelId });
      }
      await loadModels();
    } catch (e) {
