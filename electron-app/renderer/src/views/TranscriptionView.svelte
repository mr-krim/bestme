<script lang="ts">
  ...
  onMount(async () => {
    try {
      ...
      voiceCommandListener = await window.electronAPI.listen('voice-command:executed', (event) => {
  ...
  async function startRecording() {
    try {
      await window.electronAPI.invoke('audio.start_recording', {
  ...
  async function stopRecording() {
    try {
      await window.electronAPI.invoke('audio.stop_recording');
      isRecording = false;
      peakLevel = 0;
    } catch (error) {
  ...
  async function toggleVoiceCommands() {
    try {
      await window.electronAPI.invoke('transcribe.set_voice_commands_enabled', { enabled: !voiceCommandEnabled });
