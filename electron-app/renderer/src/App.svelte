<script lang="ts">
    ...
    const loadMiddlePanelItems = async (currentPanel: string) => {
      ...
      try {
        ...
        if (command) {
            ...
            const items = await window.electronAPI.invoke<PanelItem[]>(command);
  ...
  async function loadTranscriptContent(id: string | null) {
    ...
    try {
        const fullItem = await window.electronAPI.invoke<SavedTranscriptContent>('get_saved_transcript', { id });
  ...
  $: (async () => {
    if (activePanel === 'chat' && selectedItemId && selectedItemId !== currentChatFetchId) {
      ...
      try {
        const sessionContent = await window.electronAPI.invoke<ChatSessionContent>('get_chat_session', { id: fetchId });
  ...
  async function handleMicToggle() {
    ...
    if (isRecording) {
      ...
      try {
        await window.electronAPI.invoke('stop_transcription');
      ...
      // transcriptionText = ''; 
      try {
        await window.electronAPI.invoke('start_transcription', { 
  ...
  async function handleSaveTranscriptRequest(event: CustomEvent<{ title: string; content: string }>) {
    ...
    try {
      await window.electronAPI.invoke('save_transcript', { title, content });
  ...
  async function handleDeleteTranscriptRequest(event: CustomEvent<{ id: string }>) {
    ...
    try {
      await window.electronAPI.invoke('delete_saved_transcript', { id });
  ...
  async function handleDeleteChatSession(event: CustomEvent<{ id: string }>) {
    ...
    try {
      await window.electronAPI.invoke('delete_chat_session', { id });
  ...
  async function handleSendChatMessage(event: CustomEvent<{ sessionId: string | null; message: string }>) {
    ...
    try {
      const [newSessionId, aiMessage] = await window.electronAPI.invoke<[string, ChatMessage]>('send_chat_message', { 
        sessionId,
        userMessage: message 
      });
  ...
  async function checkBackendStatus() {
    try {
      ...
      const [cpu, mem, onlineStatus] = await Promise.all([
        window.electronAPI.invoke<number>('system.get_cpu_usage'),
        window.electronAPI.invoke<number>('system.get_memory_usage'),
        window.electronAPI.invoke<boolean>('system.get_online_status')
      ]);
  ...
  onMount(async () => {
    ...
    try {
      const [models, langs] = await Promise.all([
        window.electronAPI.invoke<string[]>('get_whisper_models'),
        window.electronAPI.invoke<[string, string][]>('get_supported_languages')
      ]);
      aiModels = models;
    ...
    try {
      ...
      const loadedConfig = await window.electronAPI.invoke<AppConfig>('get_settings');
    ...
    try {
      const updateListener = window.electronAPI.listen('transcription:update', (event) => {
        console.log('Transcription update:', event.payload);
        transcriptionText = event.payload; // Update the transcription text state
      });

      const clearListener = window.electronAPI.listen('transcription:clear', () => {
        console.log('Transcription clear event');
        transcriptionText = ''; // Clear the text
      });

      const errorListener = window.electronAPI.listen('transcribe:error', (event) => {
      ...
      const startedListener = window.electronAPI.listen('transcribe:started', () => {
          console.log('Transcription started event received - updating UI state');
          isRecording = true;
      });
      const stoppedListener = window.electronAPI.listen('transcribe:stopped', () => {
    ...
    setTimeout(async () => {
      try {
        const recording = await window.electronAPI.invoke<boolean>('is_transcribing');
  ...
  onDestroy(() => {
    ...
    eventListeners.forEach(unlisten => unwindow.electronAPI.listen());
  });
</script>
