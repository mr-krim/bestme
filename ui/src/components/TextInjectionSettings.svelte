<script lang="ts">
  ...
  onMount(async () => {
    try {
      permissions = await invoke('check_injection_permissions');
      if (permissions.has_required_privileges) {
        activeWindow = await invoke('get_active_window');
      }
    } catch (error) {
  ...
  async function toggleInjection() {
    try {
      await invoke('enable_text_injection', { enabled: !enabled });
      enabled = !enabled;
    } catch (error) {
  ...
  async function requestPermissions() {
    try {
      await invoke('request_injection_permissions');
  ...
  async function updateMode() {
    try {
      await invoke('set_injection_mode', { 
        mode: {
  ...
  async function testInjection() {
    ...
    try {
      await invoke('inject_text', { text: testText });
    } catch (error) {
