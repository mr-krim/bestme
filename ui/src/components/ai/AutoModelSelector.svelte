<script lang="ts">
  ...
  async function analyzeText() {
    ...
    try {
      characteristics = await invoke('analyze_text_characteristics', {
        text: textToAnalyze
      });
    } catch (e) {
  ...
  async function autoSelectModel() {
    ...
    try {
      selectionResult = await invoke('auto_select_model', {
        text: textToAnalyze
      });
  ...
  async function updateConfig() {
    try {
      await invoke('update_model_selector_config', { config });
      error = '';
    } catch (e) {
  ...
  async function loadSelectionHistory() {
    try {
      selectionHistory = await invoke('get_model_selection_history');
    } catch (e) {
