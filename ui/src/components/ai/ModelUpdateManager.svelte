<script lang="ts">
  ...
  onMount(async () => {
    ...
    unlistenDownloadProgress = await listen('model-download-progress', (event: any) => {
  ...
  async function checkForUpdates(force: boolean) {
    ...
    try {
      updateResult = await invoke('check_model_updates', { force });
  ...
  async function loadUpdateHistory() {
    try {
      updateHistory = await invoke('get_update_history');
    } catch (e) {
  ...
  async function updateConfig() {
    try {
      await invoke('configure_update_checker', { config });
